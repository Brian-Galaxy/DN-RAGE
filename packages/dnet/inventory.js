...
inventory.getItemList = function(player, ownerType, ownerId, isFrisk = false) {
    ...
    try {
        ...
        let sql = `SELECT * FROM items WHERE owner_id = '${ownerId}' AND owner_type = '${ownerType}'${addWhere} ORDER BY item_id DESC LIMIT 200`;
        if (ownerId == 0 && ownerType == 0)
            sql = `SELECT * FROM items WHERE DISTANCE(POINT(pos_x, pos_y), POINT(${player.position.x}, ${player.position.y})) < 2 AND owner_type = 0 ORDER BY item_id DESC LIMIT 200`;
