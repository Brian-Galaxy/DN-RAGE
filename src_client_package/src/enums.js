...
enums.get = function(_enum){
    switch (_enum) {
        case 'clothM': return _clothM;
        case 'propM': return _propM;
        case 'clothF': return _clothF;
        case 'propF': return _propF;
        case 'shopList': return _shopList;
        case 'vehicleInfo': return _vehicleInfo;
        case 'overlays': return _overlays;
        case 'printList': return _tprint;
        case 'tattooList': return _tattooList;
...
// должно быть тут, иначе никак
mp.events.add('client:enums:updateCloth1', (requestID, tattooList, printList) => {
    try {
        ...
        if (pendingRequests[requestID]) {
            pendingRequests[requestID]([tattooList, printList]);
        }
    } catch (e) {
        console.log(e);
        throw e;
...
enums.updateVehicleInfo = function(data) {
    ...
    methods.debug('_vehicleInfo' + _vehicleInfo.length);
    methods.debug('_vehicleInfo' + _vehicleInfo.length);
    methods.debug('_vehicleInfo' + _vehicleInfo.length);
    methods.debug('_vehicleInfo' + _vehicleInfo.length);
    methods.debug('_vehicleInfo' + _vehicleInfo.length);
...
enums.fractionRankList = [
    ...
    ["Cadet PA","Police Officier I","Police Officier II","Police Officier III","Police Officier III+1","Sergeant I","Sergeant II","Lieutenant I","Lieutenant II","Captain I","Captain II","Commander","Assistant Chief of Police","Chief of Police"],["Стажер","Дежурный","Младший Агент","Агент","Глава отдела","Инспектор","Зам. директора FIB","","","","","","","Директор FIB"],
...
enums.animActions = [
    ...
    ["Передать что-то по рации", "random@arrests", "generic_radio_chatter", 49], /*["Сесть 1","amb@prop_human_seat_chair@male@generic@base", "base",9],["Сесть 2","amb@prop_human_seat_chair@male@elbows_on_knees@base", "base",9],["Сесть 3","amb@prop_human_seat_chair@male@left_elbow_on_knee@base", "base",9],["Сесть 4","amb@prop_human_seat_chair@male@right_foot_out@base", "base",9],*/
