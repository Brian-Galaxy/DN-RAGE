...
// Family Pharmacy - Мишн-Роу - Фантастик-плейс
    npc.create(mp.game.joaat("a_m_m_business_01"), new mp.Vector3(317.9639, -1078.319, 29.47855), 359.3141, false, "WORLD_HUMAN_STAND_IMPATIENT");
        ...
        if (dist < _loadDist && !item.isCreate) {
            ...
            try {
                if (mp.game.streaming.hasModelLoaded(item.model)) {
                    ...
                    //methods.debug('CreatePed', item);

                    /*mp.game.invoke(methods.FREEZE_ENTITY_POSITION, item.handle, true);
                    mp.game.invoke(methods.SET_ENTITY_COORDS_NO_OFFSET, item.handle, item.pos.x, item.pos.y, item.pos.z, true, true, true);
                    //mp.game.invoke(methods.SET_PED_CAN_BE_TARGETTED, item.handle, true);
                    //mp.game.invoke(methods.SET_PED_CAN_BE_TARGETTED_BY_PLAYER, item.handle, true);
                    mp.game.invoke(methods.SET_BLOCKING_OF_NON_TEMPORARY_EVENTS, item.handle, true);
                    //mp.game.invoke(methods.TASK_SET_BLOCKING_OF_NON_TEMPORARY_EVENTS, item.handle, true);
                    mp.game.invoke(methods.SET_ENTITY_INVINCIBLE, item.handle, true);
                    //mp.game.invoke(methods.SET_PED_CAN_RAGDOLL, item.handle, false);
                    //mp.game.invoke(methods.SET_PED_CAN_EVASIVE_DIVE, item.handle, false);
                    //mp.game.invoke(methods.SET_PED_GET_OUT_UPSIDE_DOWN_VEHICLE, item.handle, false);
                    //mp.game.invoke(methods.SET_PED_AS_ENEMY, item.handle, false);
                    mp.game.invoke(methods.SET_CAN_ATTACK_FRIENDLY, item.handle, false, false);
                    mp.game.invoke(methods.SET_PED_DEFAULT_COMPONENT_VARIATION, item.handle);
                    mp.game.invoke(methods.SET_ENTITY_AS_MISSION_ENTITY, item.handle, false, true);*/
            ...
            try {
                ...
                //mp.game.invoke(methods.SET_ENTITY_AS_NO_LONGER_NEEDED, item.handle);
                //mp.game.invoke(methods.SET_PED_AS_NO_LONGER_NEEDED, item.handle);
                /*mp.game.invoke(methods.SET_ENTITY_AS_MISSION_ENTITY, item.handle, true, true);
                mp.game.invoke(methods.DELETE_ENTITY, item.handle);
