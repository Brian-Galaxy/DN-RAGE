...
let currentTask = 0;
let tasks = [
    'Войдите в дом и соберите все необходимые вещи',
    'Экипируйте телефон через инвентарь (I -> Экипировать телефон)',
    'Выйдете на улицу',
    'Возьмите канистру и заправьте транспорт',
    'Садитесь в транспорт (L - Открыть/Закрыть транспорт)',
    'Запустите двигатель (2 -> Вкл Двигатель)',
    'Выйдете из транспорта и возьмите ящик с инструментами',
    'Почините транспорт (G -> Открыть капот) далее (I - Использовать инструменты)',
    'Садитесь в транспорт',
    'Езжайте в аэропорт',
    'Поговорите с полицейским',
    'Езжайте в аэропорт по второй дороге',
];

let vehicleList = [
    ["PoliceOld1", 3197.02197265625, -4830.8232421875, 111.42011260986328, 200.616943359375, true], //TODO YANKTON
...
prolog.start = function() {
    ...
    ui.showDialog('Добро пожаловать. Вы зашли на Medium RP сервер, поэтому просим вас ознакомиться с правилами проекта и соблюдать RolePlay процесс. Мы просим вас быть вежливыми и сдержанными с другими игроками. Мы хотим чтобы каждый игрок поддерживал и уважал атмосферу на сервере. Те, кто хочет портить игру другим игрокам, будут забанены администрацией. Желаем приятной игры!', 'Информация');
    ...
    npc.create(mp.game.joaat("s_m_m_prisguard_01"), new mp.Vector3(3195.615478515625, -4843.75048828125, 111.86639404296875), 357.7933044433594, false, "WORLD_HUMAN_SMOKING");
    npc.create(mp.game.joaat("s_m_m_snowcop_01"), new mp.Vector3(3195.656005859375, -4842.25537109375, 111.86990356445312), 174.50775146484375, false, "WORLD_HUMAN_DRUG_DEALER");
    npc.create(mp.game.joaat("cs_prolsec_02"), new mp.Vector3(3198.975341796875, -4833.72998046875, 111.81517791748047), 263.877197265625, false, "WORLD_HUMAN_CLIPBOARD");
    npc.create(mp.game.joaat("s_m_y_ranger_01"), new mp.Vector3(3197.803955078125, -4821.73779296875, 112.10292053222656), 186.72738647460938, false, "WORLD_HUMAN_GUARD_STAND");
    npc.create(mp.game.joaat("s_m_m_snowcop_01"), new mp.Vector3(3198.833984375, -4811.748046875, 111.97410583496094), 10.083436012268066, false, "CODE_HUMAN_MEDIC_TIME_OF_DEATH");
    npc.create(mp.game.joaat("csb_prologuedriver"), new mp.Vector3(3198.473876953125, -4809.9091796875, 112.10059356689453), 185.67567443847656, false, "WORLD_HUMAN_DRINKING");
    npc.create(mp.game.joaat("ig_brad"), new mp.Vector3(3203.83447265625, -4822.96484375, 111.93563842773438), 100.02397918701172, false, "WORLD_HUMAN_SMOKING");
    npc.create(mp.game.joaat("u_m_m_promourn_01"), new mp.Vector3(3203.02880859375, -4819.8037109375, 111.87794494628906), 119.79261779785156, false, "WORLD_HUMAN_STAND_IMPATIENT");
    npc.create(mp.game.joaat("u_f_m_promourn_01"), new mp.Vector3(3200.32177734375, -4843.861328125, 112.0701904296875), 77.06122589111328, false, "WORLD_HUMAN_TOURIST_MOBILE");
    npc.create(mp.game.joaat("g_m_m_chigoon_01"), new mp.Vector3(3214.7138671875, -4849.59423828125, 111.74788665771484), 355.5313415527344, false, "WORLD_HUMAN_DRINKING");
    npc.create(mp.game.joaat("s_m_m_armoured_01"), new mp.Vector3(5432.6328125, -5142.80419921875, 78.19537353515625), 335.6464538574219, false, "WORLD_HUMAN_CLIPBOARD");
};
...
prolog.stop = function() {
    ...
    setTimeout(function () {
        ...
        setTimeout(function () {
            let roleIdx = user.getCache('role') - 1;
            if (roleIdx === 2) {
                ui.showDialog('По прилёту в штат San Andreas, вас ограбили, украв все деньги и документы, спустя неделю скитаний вы оказались в  Sandy Shores, чтобы начать свой путь. Первым делом езжайте в здание правительства', 'Информация');
            }
            else if (roleIdx === 0) {
                ui.showDialog('По прилёту в штат San Andreas, вас ограбили, украв сумку и документы, поэтому вам пришлось отправиться на корабле, чтобы начать свой путь', 'Информация');
            }
            else {
                ui.showDialog('По прилёту в штат San Andreas, вас ограбили, украв сумку, но вы не отчаились и на метро вы приехали к зданию правительства, чтобы начать свой путь. Первым делом езжайте в здание правительства', 'Информация');
            }
...
prolog.next = function() {
    if(isStart && currentTask === 5) {
        ...
        pickup.toolkit = true;
        pickupsPool.push(pickup);
        ui.showDialog('Вам необходимо взять набор инструментов и произвести небольшой ремонт, связи с тем, что двигатель не запускаеться.', 'Информация');
    }
    ...
    try {
        if (isStart) {
            ...
            if (currentTask === 2) {
                ...
                setTimeout(function () {
                    mp.events.callRemote('server:phone:sendMessageNumber', 'Друг', user.getCache('phone'), `Привет, так по поводу работы все готово, по прилёту дуй в здание правительства и устроишься на садовника или разнорабочего, зарабатывать первые деньги и добиваться успеха. Я в тебя верю!`);
                }, 3000)
            }
        }
    }
