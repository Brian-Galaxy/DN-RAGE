...
let questList = {
    ...
    quest_gang: {
        name: "Тёмная сторона",
        pos: new mp.Vector3(-119.74713134765625, -1770.3973388671875, 29.852500915527344),
        tasks: [
            ["E-Coin", "Найти Ламара и получить доступ к E-Coin кошельку", "Доступ к E-Corp приложению в телефоне"],
            ["Помощь", "Необходимо помочь Ламару", "$200"],
            ["Разведка", "Необходимо помочь Ламару", "$100"],
            ["Перевозка", "Необходимо помочь Ламару", "$500"],
            ["Вопросы", "Узнайте что происходит", "Доступ к ограблению домов и угону транспорта в приложении ECorp"],
...
quest.gang = function(start = -1) {
    ...
    switch (start) {
        case 0: {
            ...
            ui.showDialog('Ватсап друг, вот держи ссылку, теперь у тебя есть доступ к приложению E-Corp, мы все сейчас через него работаем, крутая штука', 'Ламар');
            break;
        }
        case 1: {
        ...
        case 3: {
            ...
            vehicles.spawnJobCar(-126.40444946289062, -1771.87060546875, 29.566505432128906, 317.9000549316406, "Speedo4", 0);
            break;
        }
        case 4: {
            user.set(qName, 5);
...
quest.role0 = function(start = -1) {
    ...
    switch (start) {
        case 0: {
            ...
            ui.showDialog('Привет, я смотрю ты только приехал, у меня для тебя работка есть, уверен лишние деньги тебе не помешают. В общем, необходимо разгрузить 30 ящиков с коробля, как будешь готов, я выдам тебе форму', 'Каспер');
            break;
        }
        case 1: {
